...
export default function Experience() {
  ...
  return (
    <section id="experience" className="py-16 sm:py-20 lg:py-24 relative overflow-hidden bg-dark-bg/60">
      
      {/* Ambient background light */}
      <div className="absolute top-1/2 left-1/4 w-64 sm:w-96 h-64 sm:h-96 bg-emerald-600/10 rounded-full blur-[100px] sm:blur-[150px] pointer-events-none"></div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        <RevealSection variant="fadeUp">
          <div className="text-center max-w-3xl mx-auto space-y-3 sm:space-y-4 mb-10 sm:mb-16">
            <div className="inline-flex items-center gap-2 px-3 sm:px-3.5 py-1 rounded-full bg-emerald-500/10 border border-emerald-500/30 text-emerald-300 text-[10px] sm:text-xs font-mono uppercase tracking-wider">
              <Briefcase className="w-3 sm:w-3.5 sm:h-3.5 text-emerald-400" />
        ...
        <RevealSection variant="fadeUp" delay={150}>
          <div className="flex justify-center mb-8 sm:mb-12">
            <div className="inline-flex p-1 sm:p-1.5 rounded-xl sm:rounded-2xl bg-dark-surface border border-white/10">
              <button
                onClick={() => setActiveTab('experience')}
                className={`px-3 sm:px-6 py-2 sm:py-2.5 rounded-lg sm:rounded-xl text-[11px] sm:text-sm font-semibold transition-all flex items-center gap-1.5 sm:gap-2 ${
          ...
          <div className="max-w-4xl mx-auto space-y-5 sm:space-y-8">
            {experienceData.map((exp, idx) => (
              <RevealSection key={idx} variant="fadeUp" delay={idx * 150}>
                <div className="glass-card p-5 sm:p-6 lg:p-8 rounded-2xl sm:rounded-3xl border border-white/10 space-y-3 sm:space-y-4 hover:border-emerald-400/40 relative group">
                  <div className="flex flex-col gap-3 border-b border-white/10 pb-3 sm:pb-4">
                    ...
                    <div className="flex flex-wrap items-center gap-2 text-[10px] sm:text-xs text-slate-400 font-mono">
                      <span className="flex items-center gap-1 bg-dark-surface px-2 sm:px-3 py-1 rounded-lg border border-white/10">
                        <Calendar className="w-3 h-3 sm:w-3.5 sm:h-3.5 text-emerald-400" /> {exp.period}
                      </span>
                      <span className="flex items-center gap-1 bg-dark-surface px-2 sm:px-3 py-1 rounded-lg border border-white/10">
                        <MapPin className="w-3 h-3 sm:w-3.5 sm:h-3.5 text-emerald-400" /> {exp.location}
                      </span>
                    </div>
                  </div>
                  ...
                  <div className="space-y-2 pt-1 sm:pt-2">
                    ...
                    <ul className="space-y-1.5 sm:space-y-2">
                      {exp.highlights.map((item, i) => (
                        <li key={i} className="flex items-start gap-2 sm:gap-2.5 text-[11px] sm:text-xs lg:text-sm text-slate-300 leading-relaxed">
        ...
        {activeTab === 'education' && (
          <div className="max-w-4xl mx-auto space-y-4 sm:space-y-6">
            {educationData.map((edu, idx) => (
              <RevealSection key={idx} variant="fadeUp" delay={idx * 150}>
                <div className="glass-card p-5 sm:p-6 lg:p-8 rounded-2xl sm:rounded-3xl border border-white/10 space-y-2.5 sm:space-y-3 hover:border-emerald-400/40 relative">
                  <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-2 sm:gap-3">
                    <div className="space-y-1">
                      <span className="text-[10px] sm:text-xs font-mono text-emerald-300 px-2 sm:px-2.5 py-0.5 rounded bg-emerald-500/10 border border-emerald-500/30 inline-block">
                        {edu.status}
                      </span>
                      <h3 className="text-base sm:text-xl font-bold text-white">{edu.degree}</h3>
                    ...
                    <div className="flex items-center gap-2 text-[10px] sm:text-xs font-mono text-slate-400">
                      <span className="bg-dark-surface px-2 sm:px-3 py-1 rounded-lg border border-white/10 flex items-center gap-1">
                        <Calendar className="w-3 h-3 sm:w-3.5 sm:h-3.5 text-emerald-400" /> {edu.period}
                      </span>
                    </div>
                  </div>

                  <p className="text-[11px] sm:text-xs lg:text-sm text-slate-400 pt-2 border-t border-white/5">
                    {edu.description}
                  </p>
                </div>
              </RevealSection>
